...
export function Navbar() {
  ...
  return <header className="nav-wrap"><nav className="nav shell" aria-label="Primary navigation">
    <a href="#home" className="brand brand-identity" aria-label="Mohamed home"><span className="brand-monogram"><Image src="/mohamed-monogram.svg" alt="" width={82} height={48} priority /></span><span className="brand-copy"><b>MOHAMED 37<span className="brand-dot"></span></b><small>FULL STACK DEVELOPER</small></span></a>
    <div className="nav-links">{links.map((link, index) => <a  key={link.href} href={link.href}>{link.label}</a>)}</div>
